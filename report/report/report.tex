...
\( 33 \times 33 \times 33 \) and get \( 2 \times 2 \times 2 \) prediction for center region.
...
we just add \( m \in \mathbb{N} \) pixel to input along one or several dimension, but every downsampling layer divide a
...
The second approach is a training of the variational autoencoder (VAE) \cite{kingma2013auto} where we mask the center voxel
(optionally with padding, i.e. some center region) and reconstruct the whole input (including the masked
center region) using VAE framework. This approach is adapted from \cite{woo2023convnext} but we
mask interesting for us center voxel/region and usually use different network architecture.
